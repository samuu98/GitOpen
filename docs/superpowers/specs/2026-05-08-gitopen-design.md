...
> **2026-05-08 amendment.** The original Photino.Blazor stack proved unable to
> deliver the IDE-grade fluidity the user expected (drag/resize lag through
> the WebView2 bridge, Blazor render overhead on the commit graph). Tagged
> the C# implementation as `slice-1-csharp-photino` and pivoted to Flutter
> for the next iteration. The architectural decisions (layering, hybrid
> libgit2 + git CLI, multi-tab UX, lane algorithm, multi-window roadmap)
> carry over unchanged — only the implementation language and toolkit
> change. Avalonia was offered as a .NET alternative and explicitly
> rejected by the user.

| Layer | Choice |
|---|---|
| Runtime | **Flutter (latest stable) + Dart** |
| Desktop host | Flutter Desktop (Windows + Linux) |
| UI | Flutter widgets, custom design system (no Material defaults) |
| Window/chrome | **`bitsdojo_window`** for chromeless window with native drag/resize |
| Git read operations | **`libgit2dart`** (Dart FFI binding to libgit2) |
| Git write operations | **`git` CLI** via `Process.run` (shell-out) |
| Persistence | **`drift`** (Flutter ORM over SQLite) |
| State management | **`flutter_riverpod`** |
| Logging | **`logger`** package (file + console output) |
| Commit graph rendering | **`CustomPainter`** on Skia Canvas (no SVG) |
| Tests | `flutter_test` (widget + unit) |
| CI | GitHub Actions matrix (windows-latest, ubuntu-latest) |
| Packaging | MSIX (Windows), AppImage (Linux) |
| License | MIT |

Rationale: Flutter Desktop renders via Skia and uses native window APIs for
chrome operations, so drag/resize and pointer interactions are 60fps native.
Dart is a manageable ramp-up (similar in feel to TypeScript) and the
layering means the cost is concentrated in re-implementing the Domain and
Application layers in Dart and writing a fresh widget tree — no UI logic
needs to be invented from scratch since the design is already validated by
the C# experiment.

## 3. Solution Structure

```
gitopen/                          Flutter project root (== repo root)
├─ pubspec.yaml                   Dart deps + assets
├─ lib/
│  ├─ domain/                     Pure-Dart records: Repo, Commit, Branch, ...
│  ├─ application/                Services, contracts, lane layout, workspace mgr
│  ├─ infrastructure/             libgit2dart wrapper, git CLI, drift schema, FS watcher
│  └─ ui/                         Widgets, screens, design tokens
│     ├─ shell/                   TitleBar, Sidebar, ResizeHandles
│     ├─ commit_graph/            CommitGraphPanel, CommitRow, CustomPainter
│     ├─ bottom_panel/            CommitDetails, DiffView, FileTreeView
│     └─ theme/                   Colors, typography, spacing, custom theme
├─ test/                          flutter_test (widget + unit)
├─ windows/                       Generated by `flutter create`
├─ linux/                         Generated by `flutter create`
├─ docs/superpowers/              Spec + plans (carry over from C# era)
├─ .github/workflows/             CI matrix Win + Ubuntu
└─ legacy/                        (optional) the C# experiment, kept for reference
```
